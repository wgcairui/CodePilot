...
import { insertAuditLog, getSetting } from '../../db';
...
export class DiscordAdapter extends BaseChannelAdapter {
  ...
  async start(): Promise<void> {
    ...
    const token = getSetting('bridge_discord_bot_token') || '';
  ...
  getPreviewCapabilities(chatId: string): PreviewCapabilities | null {
    // Global kill switch
    if (getSetting('bridge_discord_stream_enabled') === 'false') return null;
  ...
  isAuthorized(userId: string, chatId: string): boolean {
    const allowedUsers = getSetting('bridge_discord_allowed_users') || '';
    const allowedChannels = getSetting('bridge_discord_allowed_channels') || '';
  ...
  private async processMessage(message: any): Promise<void> {
    ...
    if (isGuild) {
      const allowedGuilds = (getSetting('bridge_discord_allowed_guilds') || '')
      ...
      const policy = getSetting('bridge_discord_group_policy') || 'open';
      ...
      const requireMention = getSetting('bridge_discord_require_mention') === 'true';
    ...
    const imageEnabled = getSetting('bridge_discord_image_enabled') !== 'false';
    const maxSize = parseInt(getSetting('bridge_discord_max_attachment_size') || '', 10) || DEFAULT_MAX_ATTACHMENT_SIZE;
